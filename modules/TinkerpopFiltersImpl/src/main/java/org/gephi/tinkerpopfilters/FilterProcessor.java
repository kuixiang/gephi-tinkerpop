    ...
    public Graph process(AbstractQueryImpl query, GraphModel graphModel,
        ...
        for (int i = 0; i < tree.length; i++) {
            ...
            if ((filter instanceof ComplexFilter) &&
                (!isLeaf && input[0].getNodeCount() > 0 || isLeaf)) {
                ComplexFilter cf = (ComplexFilter) filter;
                if (progressTicket != null) {
                    progressTicket.setDisplayName(cf.getName());
                } else if (progressTicketProvider != null){
                    ProgressTicket progressTicketNew = progressTicketProvider
                        .createTicket(cf.getName(), null);
                    Progress.start(progressTicketNew);
                }
                q.setResult(cf.filter(input[0], leafOnly, isLeaf));
                if (progressTicketProvider != null && progressTicket == null) {
