 ...
 */
package org.gephi.tinkerpopfilters;
...
import org.gephi.tinkerpopfilters.api.PropertyExecutor;
import org.gephi.tinkerpopfilters.api.Query;
import org.gephi.tinkerpopfilters.api.Range;
import org.gephi.tinkerpopfilters.FilterThread.PropertyModifier;
import org.gephi.tinkerpopfilters.spi.*;
...
public class FilterControllerImpl implements FilterController, PropertyExecutor {
    ...
    public GraphView filter(Query query) {
        ...
        GraphModel graphModel = Lookup.getDefault().lookup(GraphController.class).getModel();
        ProgressTicketProvider progressTicketProvider =
            Lookup.getDefault().lookup(ProgressTicketProvider.class);
        Graph result = processor.process((AbstractQueryImpl) query, graphModel,
            progressTicketProvider, null);
    ...
    public void setValue(FilterProperty property, Object value, Callback callback) {
        if (model != null) {
            ...
            if ((filterThread = model.getFilterThread()) != null &&
                model.getCurrentQuery() == rootQuery &&
                !Thread.currentThread().equals(filterThread)) {
